...
def make_request(baseurl, params):
    ...
    dict
        ...
        a dictionary
    '''
    response_json = requests.get(baseurl, params=params, auth=oauth)
...
def find_most_common_cooccurring_hashtag(tweet_data, hashtag_to_ignore):
    ...
    '''
    fixed_hashtag_to_ignore = hashtag_to_ignore[1:].lower() #getting rid of the # and make lowercase so I can use it in my loop
    all_tweets = tweet_data['statuses'] #get the list of 100 tweets
    hashtagcounter = {} #a dictionary to keep track of cooccuring hashtags. key is hashtag, value is num times it occurs

    for tweet in all_tweets: #for each tweet in this list, which are dictionaries representing a specific tweet
        hashtag_list = tweet['entities']['hashtags'] #this gets us the list of dictionaries, where each dictionary represents one of the hashtags on that tweet
        for tag in hashtag_list: #now we look at each hashtag in the list
            if tag['text'].lower() != fixed_hashtag_to_ignore and tag['text'].lower() not in hashtagcounter.keys(): #if this is a cooccuring hashtag, and we didn't already encounter it
                hashtagcounter[tag['text'].lower()] = 1
            elif tag['text'].lower() != fixed_hashtag_to_ignore and tag['text'].lower() in hashtagcounter.keys(): #if cooccuring, but we already encountered it, add one to its value
                hashtagcounter[tag['text'].lower()] +=1

    #loop to determine which key occurs the same time
    #use the hashtag counter dictionary key value pairs to determine this
    max_num = 0
    most_common_hashtag = None
    for key in hashtagcounter.keys():
        if hashtagcounter[key] > max_num:
            max_num = hashtagcounter[key]
            most_common_hashtag = key

    return f"#{most_common_hashtag}"
